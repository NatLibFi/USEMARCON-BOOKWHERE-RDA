...
if ( $target_language !~ /^(eng|fin|swe)$/ ) {
...
sub create_tbl_file_contents($$$){
	...
	my %rows;
	foreach my $code ( @rda_types ) {
	    if ( $tag eq '336' && length($code) != 3 ) { next; }
	    if ( $tag eq '337' && length($code) != 1 ) { next; }
	    my $rhs = &normalize_term($code);
	    foreach my $language ( @languages ) {
		my $curr_rhs = $rhs;
		if ( defined($rda{$code}{$language}) ) {
		    my $cand = $rda{$code}{$language};
		    my $lhs = &normalize_term($cand);
		    if ( defined($rows{$lhs}) && $rows{$lhs} ne $curr_rhs ) {
			die($curr_rhs);
			$curr_rhs = "__ERROR__";
		    }
		    $rows{$lhs} = $curr_rhs;
...
print "Mappings for $target_tag}\$$target_subfield_code\n";
print "Autogenerated file using $0!\n\n";
